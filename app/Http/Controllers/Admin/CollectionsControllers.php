...
class CollectionsControllers extends Controller
{
    ...
    public function create_graphs(Request $request)
    {
        try {
            $oltId=$request->id;
            $pons = OltConfig::where("id", "=", $oltId)->first("PONS");

            for ($poid = 1; $poid <= $pons->PONS; $poid++)
            {
                GraphPonsConfig::create([
                    "ID_OLT_GRAPH" => $oltId,
                    "PORT" => "gpon 1/1/$poid",
                    "NAME_GRAPH" => "Porta 1/1/$poid"
                ]);

                $olt=OltConfig::where("id", "=", "$oltId")->first();
                $olt->CREATE_GRAPH = true;
                $olt->save();
            }

            return $this->success($this->getMessage("appsuccess", "SuccessEquipmentCreatedGraphs"),  $code=200);

        } catch (\PDOException $err) {
            dd($err);
            return $this->error($this->getMessage("apperror", "ErrorEquipmentCreatedGraphs"),  $code=400);;
        }
    }
    ...
    public function save_dbm_collections(Request $request)
    {
        ...
        $orgName = $request->data_file->getClientOriginalName();
    ...
    public function execute_dbm_import_task(Request $request)
    {
        ...
        try {
            $process = new Process(["python3", env("PATH_SCRIPTS") . "app.py", $task->path]);
            $process->run();
